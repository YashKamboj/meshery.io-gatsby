...
const data = [
  {
    name:"Community Calendar",
     description: "Join the weekly meetings by subscribing to our community calendar",
  ...
  },
  {
    name:"Join the Conversation",
     description: "Ask questions, find answers and share knowledge on our Discussion Forum",
     link: "https://discuss.layer5.io",
  },
  {
    name:"Community Guide",
     description: "Read the Welcome Guide to familiarize yourself with the community",
...
const Projects = () => {
  return (
    <ProjectItemWrapper id="projects">
      ...
      {data.map((data) =>{
        return(
        <a className="card" href={data.link} target="_blank" rel="noreferrer">
          <h3>{data.name}</h3>
          <p className="small">{data.description}</p>
          <img className="go-corner" src={projectImage3} alt="projectImage" href={data.link} />
        </a>
        );
})}
      </div>
